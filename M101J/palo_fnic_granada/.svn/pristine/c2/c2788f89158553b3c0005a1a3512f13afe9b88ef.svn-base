...
int red_hw_sat_reg_test (int inst, const int seed, const int check_default, unsigned int* addr_error,unsigned  int* exp_data,unsigned  int* got_data) {
        ...
        const int TABLE_SIZE = sizeof(red_hw_sat_reg_table)/sizeof(table_entry_t);
        RED_SRAND(seed);
        for (i=0; i<TABLE_SIZE; ++i) {
                const int byte_address = red_hw_sat_reg_table[i].word_addr<<2;
                *addr_error=byte_address;
                ...
                if (check_default) {
                    ...
                    if (read_data!=red_hw_sat_reg_table[i].init) {
                        *exp_data=red_hw_sat_reg_table[i].init;
                        *got_data=read_data;
                ...
                const unsigned int MASK = red_hw_sat_reg_table[i].mask;
