...
#if defined(POWER_SUPPLY_SUPPORTS_D5_D9_READ)
...
int pmbus_cmn_state (int fd, uint8_t bus, uint8_t i2c_addr)
{
	int rc = 0;
...
int ltc_state (int fd, uint8_t bus, uint8_t i2c_addr)
{
	...
	rc = pmbus_cmn_state (fd, bus, i2c_addr);
	return (rc);
}
...
int pwr_state (int fd, uint8_t bus, uint8_t i2c_addr)
{
	...
	rc = pmbus_direct_rd (fd, bus, i2c_addr, PMBUS_READ_VOUT, "Voltage Out Rd", "Volts");
	if(rc) return(rc);
...
int pmbus_state (int fd, uint8_t bus, uint8_t i2c_addr)
{
	...
	rc = pmbus_scale_mode_rd (fd, bus, i2c_addr, PMBUS_READ_VOUT, "Voltage Out Rd", 
					"Volts", 1000);
	if(rc) return(rc);
